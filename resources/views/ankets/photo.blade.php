...
<table class="fotoComments">
	...
	<tr>
		<td class="fotoCommPics"><a href="{{route('ank.id', $item->user_id)}}">
			<img alt="{{ $item->user->name }},{{ $item->user->user_age }} {{ $item->user->user_age_type }},{{ $item->user->city->name }}" src="{{ (new FileService)->outPicture($item->user_photo_id, $item->user->user_sex) }}" /></a></td>
		<td>
			<h4>
