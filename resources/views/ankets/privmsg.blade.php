...
<form name="anketa" action="{{route('privmsg.delete')}}" method="post">
	...
	<table class="Usermess">
		...
		<tr @if ($loop->index % 2 == 0)class="pinkRow"@endif>
			...
			<td class="UsermessCol2"><a class="ankFotosPics" href="{{route('ank.id', $item->user_id)}}"><img alt="{{ $item->user_mes->name }},{{ $item->user_mes->user_age_str }},{{ $item->user_mes->city->name }}" src="{{ (new FileService)->outPicture($item->photo_main, $item->user_mes->user_sex) }}" /></a></td>
