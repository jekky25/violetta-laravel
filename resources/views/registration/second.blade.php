...
чтобы узнать для чего вы, собственно говоря, здесь. Ну и про <strong>языки</strong> не забудьте. А то вдруг окажется, что ваш спутник или спутница говорит на совершенно непонятном для вас языке.</p>
	...
	<table class="mrg6" width="100%">
		<tr>
			...
			<td width="50%"><x-select name="body" :obj="$fields['body']" userProp="{{ old('body', $userData->body) }}" /></td>
		</tr>
		<tr>
			<td align="right">Рост:</td>
			<td><x-select name="height" :obj="$fields['height']" type=I :userProp="$userData->height" measure="см" /></td>
		</tr>
		<tr>
			<td align="right">Вес:</td>
			<td><x-select name="weight" :obj="$fields['weight']" type=I :userProp="$userData->weight" measure="кг" /></td>
		</tr>
		<tr>
			<td align="right">Цвет волос:</td>
			<td><x-select name="hair_color" :obj="$fields['hairColor']" userProp="{{ old('hair_color', $userData->hair_color) }}" /></td>
		</tr>
		<tr>
			<td	align="right">Тип волос:</td>
			<td><x-select name="hair_type" :obj="$fields['hairType']" userProp="{{ old('hair_type', $userData->hair_type) }}" /></td>
		</tr>
		<tr>
			<td align="right">Глаза:</td>
			<td><x-select name="eyes" :obj="$fields['eyes']" userProp="{{ old('eyes', $userData->eyes) }}" /></td>
		</tr>
	</table>
	<p class="pad2"></p>
	...
	<table class="mrg6" width="100%">
		<tr>
			...
			<td><x-select name="education" :obj="$fields['education']" userProp="{{ old('education', $userData->education) }}" /></td>
		</tr>
		<tr>
			<td align="right">Отношение к сигаретам:</td>
			<td><x-select name="smoke" :obj="$fields['smoke']" userProp="{{ old('smoke', $userData->smoke) }}" /></td>
		</tr>
		<tr>
			<td align="right">Отношение к спиртному:</td>
			<td><x-select name="alcohol" :obj="$fields['alcohol']" userProp="{{ old('alcohol', $userData->alcohol) }}" /></td>
		</tr>
		<tr>
			<td align="right">Семейное положение:</td>
			<td><x-select name="family_status" :obj="$fields['familyStatus']" userProp="{{ old('family_status', $userData->family_status) }}" /></td>
		</tr>
		<tr>
			<td align="right">Дети:</td>
			<td><x-select name="children" :obj="$fields['children']" userProp="{{ old('children', $userData->children) }}" /></td>
		</tr>
		<tr>
			<td align="right">Материальная поддержка:</td>
			<td><x-select name="help_money" :obj="$fields['helpMoney']" userProp="{{ old('help_money', $userData->help_money) }}" /></td>
		</tr>
		<tr>
			<td align="right">Интересы:</td>
			<td><x-select name="interests[]" :obj="$fields['interests']" multiple="true" size="10" userProp="{{ old('interests', $userData->interests) }}" /></td>
		</tr>
	</table>
	<p class="pad2"></p>
