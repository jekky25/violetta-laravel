...
{{ csrf_field() }}
	...
	<x-checkbox-container name="partner_body[]" :obj="$fields['partnerBody']" title="Телосложение:" topClass="pad2"></x-checkbox-container>
	...
	<x-checkbox-container name="partner_languages[]" :obj="$fields['partnerLanguages']" title="Знание языков:" colums="2"></x-checkbox-container>
	<x-checkbox-container name="partner_alcohol[]" :obj="$fields['partnerAlcohol']" title="Отношение к спиртному:" colums="2"></x-checkbox-container>
	<x-checkbox-container name="partner_smoke[]" :obj="$fields['partnerSmoke']" title="Отношение к сигаретам:" colums="2"></x-checkbox-container>
	<x-checkbox-container name="partner_education[]" :obj="$fields['partnerEducation']" title="Образование:" colums="2" topClass="pad2"></x-checkbox-container>
...
регион, укажите страну и регион соответственно.</p>
	...
	<div class="form-row">
		...
		<x-select name="partner_region" id="region" :obj="$fields['partnerRegion']" userProp="{{ old('partner_region', $userData->partner_region) }}">
			<x-slot:addition>onchange="updateSelect('city', this.value, 'cities');"</x-slot:addition>
		</x-select>
	</div>
	<div class="form-row pad2">
		<label for="city">город</label>
		<x-select name="partner_city" id="city" :obj="$fields['partnerCity']" userProp="{{ old('partner_city', $userData->partner_city) }}" />
	</div>	
