...
//print_r ($user->data['user_posts']);


//print_r ($_POST['confirm_code']);
//exit;
...
if ($user->data['user_posts'] <= 10) {
  ...
  if ((strstr ($message_parser->message, '<a') || strstr ($message_parser->message, '[url')) && ((time() - $user->data['user_regdate']) < (60*60*24*7)))
  { 
    $error[] = $user->lang['UBERITE_SSILKI'];
  }
...
$capcha_img = !empty ($capcha_img) ? $capcha_img : 0;
