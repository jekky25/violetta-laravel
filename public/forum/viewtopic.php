...
*
* @package phpBB3
* @version $Id: viewtopic.php 9022 2008-10-16 19:03:02Z toonarmy $
...
if (!$topic_data)
{
	...
	{
		redirect(append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . ".html"));
	}
...
if (isset($_GET['e']))
{
	...
	$redirect_url = append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . ".html");
...
$pristavit = (strlen($u_sort_param) || ($highlight_match)) ? ('?' . ((strlen($u_sort_param)) ? "$u_sort_param" : '') . (($highlight_match) ? ((strlen($u_sort_param)) ? "&amp;" : '') . "hilit=$highlight" : '')): '';
...
//$viewtopic_url_my = append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . "_s" . $start . ".html" . $pristavit);
$viewtopic_url = append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . "_s" . $start . ".html" . $pristavit);
...
$pagination = generate_pagination(append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . ".html", ((strlen($u_sort_param)) ? "$u_sort_param" : '') . (($highlight_match) ? (((strlen($u_sort_param)) ? "&amp;" : '') . "hilit=$highlight") : '')), $total_posts, $config['posts_per_page'], $start);
//$pagination = generate_pagination(append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . (($start > 0) ? ("_s" . $start) : "") . ".html", ((strlen($u_sort_param)) ? "$u_sort_param" : '') . (($highlight_match) ? (((strlen($u_sort_param)) ? "&amp;" : '') . "hilit=$highlight") : '')), $total_posts, $config['posts_per_page']);
...
$template->assign_vars(array(
	...
	'S_TOPIC_ACTION' 		=> append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . "_s" . $start . ".html"),
	...
	'U_TOPIC'				=> "{$server_path}topic_" . $forum_id . "_" . $topic_id . ".html",
	'U_FORUM'				=> $server_path,
	'U_VIEW_TOPIC' 			=> $viewtopic_url,
	'U_VIEW_FORUM' 			=> append_sid("{$phpbb_root_path}forum_" . $forum_id . ".html"),
	'U_VIEW_OLDER_TOPIC'	=> append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . "_previous.html"),
	'U_VIEW_NEWER_TOPIC'	=> append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . "_next.html"),
//	'U_PRINT_TOPIC'			=> ($auth->acl_get('f_print', $forum_id)) ? $viewtopic_url . '&amp;view=print' : '',
	'U_PRINT_TOPIC'			=> ($auth->acl_get('f_print', $forum_id)) ? $viewtopic_url . ((strlen($pristavit)) ? '&amp;' : '?') . 'view=print' : '',
	...
	'U_BOOKMARK_TOPIC'		=> ($user->data['is_registered'] && $config['allow_bookmarks']) ? $viewtopic_url . ((strlen($pristavit)) ? '&amp;' : '?') . 'bookmark=1&amp;hash=' . generate_link_hash("topic_$topic_id") : '',
...
if (!empty($topic_data['poll_start']))
{
	...
	if ($update && $s_can_vote)
	{
		...
		{
			$redirect_url = append_sid("{$phpbb_root_path}topic_" . $forum_id . "_" . $topic_id . "_s" . $start . ".html");
	...
	$template->assign_vars(array(
		...
		'U_VIEW_RESULTS'	=> $viewtopic_url . ((strlen($pristavit)) ? '&amp;' : '?') . 'view=viewpoll')
	);
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'L_IGNORE_POST'		=> ($row['hide_post']) ? sprintf($user->lang['POST_BY_FOE'], get_username_string('full', $poster_id, $row['username'], $row['user_colour'], $row['post_username']), '<a href="' . $viewtopic_url . ((strlen($pristavit)) ? '&amp;' : '?') . "p={$row['post_id']}&amp;view=show#p{$row['post_id']}" . '">', '</a>') : '',
	);
