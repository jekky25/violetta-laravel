		...
		switch ($mode)
		{
			case 'report_details':
				...
				$template->assign_vars(array(
					...
					'U_VIEW_FORUM'				=> append_sid("{$phpbb_root_path}forum_" . $post_info['forum_id'] . ".html"),
				...
				if (sizeof($report_ids))
				{
					...
					while ($row = $db->sql_fetchrow($result))
					{
						...
						$template->assign_block_vars('postrow', array(
							'U_VIEWFORUM'				=> (!$global_topic) ? append_sid("{$phpbb_root_path}forum_" . $row['forum_id'] . ".html") : '',
...
function close_report($report_id_list, $mode, $action)
{
	...
	else
	{
		...
		{
			$return_forum = sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$phpbb_root_path}forum_" . current($forum_ids) . ".html") . '">', '</a>') . '<br /><br />';
		}
		$return_topic = '';
